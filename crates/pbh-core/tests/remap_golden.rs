...
fn teredo_address_also_bans_the_embedded_client_ipv4() {
    ...
    got.sort();
    assert_eq!(got, vec!["192.0.2.45", "2001:0:4136:e378:8000:63bf:3fff:fdd2"]);
}
...
fn peer_address_translation_follows_upstream_order() {
    ...
    assert_eq!(translate_peer_ip("::ffff:1.2.3.4", 6881, &c), ("1.2.3.4".to_string(), 6881));
    // NAT64 -> 内嵌 IPv4（端口不变）
    assert_eq!(translate_peer_ip("64:ff9b::1.2.3.4", 6881, &c), ("1.2.3.4".to_string(), 6881));
    // 普通地址原样返回
    assert_eq!(translate_peer_ip("8.8.8.8", 6881, &c), ("8.8.8.8".to_string(), 6881));
    assert_eq!(
    ...
    let teredo_on = IpRemapConfig { teredo: true, ..IpRemapConfig::default() };
    assert_eq!(
...
fn nat64_extraction_respects_enabled_flag_and_prefix_list() {
    let off = IpRemapConfig {
        nat64: Nat64Config { enabled: false, ..Nat64Config::default() },
