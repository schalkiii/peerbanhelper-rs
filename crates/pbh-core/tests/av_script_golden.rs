...
fn load_single(script_file: &str) -> ExpressionEngine {
    ...
    let dir = std::env::temp_dir().join(format!("pbh_av_golden_{}_{}", std::process::id(), n));
...
fn gopeed_dev_with_valid_prefix_passes() {
    ...
    assert_eq!(
        decide(
            &m,
            &peer(Some("Gopeed Dev 1.2.3"), Some("-gp0001-x"), "1.2.3.4")
        ),
        PeerAction::NoAction
    );
}
...
fn gopeed_other_clients_and_blank_fields_pass() {
    ...
    assert_eq!(
        decide(
            &m,
            &peer(Some("qBittorrent/4.3.9"), Some("-qB4390-"), "1.2.3.4")
        ),
        PeerAction::NoAction
    );
    // clientName 为空 / peerId 为空（上游对 null 会抛异常按 pass 兜底，结果一致）
    assert_eq!(
        decide(&m, &peer(None, Some("-gp0001-x"), "1.2.3.4")),
        PeerAction::NoAction
    );
    assert_eq!(
        decide(&m, &peer(Some("Gopeed Dev"), None, "1.2.3.4")),
        PeerAction::NoAction
    );
}
...
fn table_matches_pass_and_non_table_clients_pass() {
    ...
    assert_eq!(
        decide(
            &m,
            &peer(Some("BitComet 2.0"), Some("-BC0001-x"), "1.2.3.4")
        ),
        PeerAction::NoAction
    );
    assert_eq!(
        decide(
            &m,
            &peer(Some("aria2explorer 1.0"), Some("-ae0001-x"), "1.2.3.4")
        ),
        PeerAction::NoAction
    );
    assert_eq!(
...
fn ipv6_without_feature_segment_passes() {
    ...
    assert_eq!(
        decide(
            &m,
            &peer(Some("aria2/1.36"), Some("-ae0001-x"), "2001:db8::5")
        ),
        PeerAction::NoAction
    );
    assert_eq!(
...
fn transmission_294_on_loopback6_is_banned() {
    ...
    assert_eq!(
        ban_reason(&m, &p),
        "Transmission 2.94 (IPV6 ::1) 多拨伪装吸血"
    );
}
...
fn transmission_other_versions_and_addresses_pass() {
    ...
    assert_eq!(
        decide(
            &m,
            &peer(Some("Transmission 2.94"), Some("-TR2960-x"), "2001:db8::5")
        ),
        PeerAction::NoAction
    );
    ...
    assert_eq!(
        decide(
            &m,
            &peer(Some("Transmission 3.00"), Some("-TR3000-x"), "::1")
        ),
        PeerAction::NoAction
    );
    // clientName 为空
