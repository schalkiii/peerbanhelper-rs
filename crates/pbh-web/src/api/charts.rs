...
/// 前端数据展示为空表而非报错。
pub async fn geo_ip(State(_): State<AppState>, Query(_): Query<HashMap<String, String>>) -> Response {
    let data = json!({
        "city": {},
        "isp": {},
        "province": {},
...
pub async fn trend(
    ...
    let connected = state.db.peer_first_seen_trend(start, end, downloader.as_deref()).unwrap_or_default();
    // 封禁趋势：ban_logs
    let banned = state.db.ban_trends(start, end, downloader.as_deref()).unwrap_or_default();
    let data = json!({
...
pub async fn session_analyse(
    ...
    let start = params.get("startTime").and_then(|v| v.parse::<i64>().ok()).unwrap_or(0);
    let end = params.get("endTime").and_then(|v| v.parse::<i64>().ok()).unwrap_or(now_ms());
    let downloader = params.get("downloader").map(|s| s.to_string()).filter(|s| !s.is_empty());
    let rows = state
        .db
