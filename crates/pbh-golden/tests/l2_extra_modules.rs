...
fn anti_vampire() -> AntiVampire {
    ...
    AntiVampire::new(AntiVampireSettings {
...
fn anti_vampire_xunlei_preset_rules() {
    ...
    assert_eq!(
        m.check("d", &downloading, &peer_0019, &ctx(0)).action,
        PeerAction::NoAction
    );
    ...
    assert_eq!(
        m.check("d", &downloading, &by_name, &ctx(0)).action,
        PeerAction::NoAction
    );
    assert_eq!(
        m.check("d", &seeding, &by_name, &ctx(0)).action,
        PeerAction::Ban
    );
    ...
    assert_eq!(
        m.check("d", &downloading, &old_by_name, &ctx(0)).action,
        PeerAction::Ban
    );
    ...
    assert_eq!(
        m.check(
            "d",
            &seeding,
            &peer("5.5.5.5", "-qB4700-xxxxxxxxxxxx"),
            &ctx(0)
        )
        .action,
        PeerAction::NoAction
    ...
    hs.dl_speed = 0;
    assert_eq!(
        m.check("d", &downloading, &hs, &ctx(0)).action,
        PeerAction::Ban
    );
}
...
fn anti_vampire_disabled_preset_never_bans() {
    ...
    assert_eq!(
        m.check(
            "d",
            &torrent(),
            &peer("1.1.1.1", "-XL0018-abcdefghijkl"),
            &ctx(0)
        )
        .action,
        PeerAction::NoAction
...
fn ptr_blacklist_matches_reverse_dns_against_rules() {
    ...
    let m = ptr_blacklist(
        r#"{"method":"EQUALS","content":"example.com"}"#,
        Some("example.com"),
    );
    ...
    let r = m.check("d", &t, &p, &ctx(0));
    assert_eq!(r.action, PeerAction::Ban);
    assert!(Translator::embedded()
        .render(r.reason_key.as_ref().unwrap(), "zh_cn")
        .contains(".example.com"));

    // 未命中 -> pass
    let m = ptr_blacklist(
        r#"{"method":"CONTAINS","content":"evil"}"#,
        Some("host.example.com"),
    );
...
fn reverse_dns_names_follow_rfc() {
    ...
    assert_eq!(
        reverse_dns_name("1.2.3.4").as_deref(),
...
fn idle_protection_bans_after_max_idle_time() {
    ...
    // 做种任务：首次记录起点
    assert_eq!(
        m.check("d", &seeding, &p, &ctx(0)).action,
        PeerAction::NoAction
    );
    // 未超过最大空闲时间 -> 仍放行
    assert_eq!(
...
fn idle_protection_resets_on_activity() {
    ...
    fast.up_speed = 1024;
    assert_eq!(
        m.check("d", &seeding, &fast, &ctx(100_000)).action,
        PeerAction::NoAction
    );
    assert_eq!(m.tracked_peers(), 0, "速度达标会移除空闲记录");
    // 重新开始计时，因此 300_001 时还不算超时
    assert_eq!(
        m.check("d", &seeding, &p, &ctx(300_001)).action,
        PeerAction::NoAction
    );
    assert_eq!(
        m.check("d", &seeding, &p, &ctx(600_002)).action,
        PeerAction::Ban
    );
}
...
fn idle_protection_resets_on_average_speed_within_window() {
    ...
    assert_eq!(
        m.check("d", &seeding, &uploaded_more, &ctx(1_000)).action,
        PeerAction::NoAction
    );
    ...
    // 之后重新累计，从 1_001 起算
    assert_eq!(
        m.check("d", &seeding, &p, &ctx(1_001)).action,
        PeerAction::NoAction
    );
    assert_eq!(
        m.check("d", &seeding, &p, &ctx(301_003)).action,
        PeerAction::Ban
    );
}
...
fn idle_protection_respects_protect_mode_and_peer_flags() {
    ...
    assert_eq!(
        no_flags
            .check("d", &downloading, &p_no_flags, &ctx(0))
            .action,
        PeerAction::NoAction
    );
    assert_eq!(
        no_flags
            .check("d", &downloading, &p_no_flags, &ctx(1_000_000))
            .action,
        PeerAction::NoAction,
    ...
    assert_eq!(
        no_flags
            .check("d", &downloading, &interested, &ctx(0))
            .action,
        PeerAction::NoAction
    );
    ...
    assert_eq!(
        seeding_only
            .check("d", &downloading, &p, &ctx(1_000_000))
            .action,
        PeerAction::NoAction
    );
    ...
    p2.flags = None;
    assert_eq!(
        always.check("d", &downloading, &p2, &ctx(0)).action,
        PeerAction::NoAction
    );
    assert_eq!(
        always.check("d", &downloading, &p2, &ctx(400_000)).action,
        PeerAction::Ban
    );
}
...
fn auto_range_ban_chain_bans_peers_in_the_same_prefix() {
    ...
    // 不同 /30 -> 不封
    assert_eq!(
        m.check("d", &t, &peer("1.2.3.8", "-qB4700-xxxxxxxxxxxx"), &ctx(0))
            .action,
        PeerAction::NoAction
    );
    // 自身已在封禁表中 -> 跳过（由其它模块负责）
    assert_eq!(
        m.check("d", &t, &peer("1.2.3.4", "-qB4700-xxxxxxxxxxxx"), &ctx(0))
            .action,
        PeerAction::NoAction
    );
    // 不同地址族 -> 不封
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("2001:db8::1", "-qB4700-xxxxxxxxxxxx"),
            &ctx(0)
        )
        .action,
        PeerAction::NoAction
...
fn auto_range_ban_ignores_disconnect_only_bans_and_handshaking_peers() {
    ...
    let t = torrent();
    assert_eq!(
        m.check("d", &t, &peer("9.9.9.8", "-qB4700-xxxxxxxxxxxx"), &ctx(0))
            .action,
        PeerAction::NoAction,
...
fn auto_range_ban_uses_ipv6_prefix() {
    ...
    let r = m.check(
        "d",
        &t,
        &peer("2001:db8:1:2::9", "-qB4700-xxxxxxxxxxxx"),
        &ctx(0),
    );
    assert_eq!(r.action, PeerAction::Ban);
    let t9 = Translator::embedded();
    ...
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("2001:db8:2::9", "-qB4700-xxxxxxxxxxxx"),
            &ctx(0)
        )
        .action,
        PeerAction::NoAction
...
fn multi_dialing_blocks_the_ip_that_exceeds_the_tolerance() {
    ...
    assert_eq!(
        m.check("d", &t, &peer("10.0.0.1", "-qB4700-aaaaaaaaaaaa"), &ctx(0))
            .action,
        PeerAction::NoAction
    );
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("10.0.0.2", "-qB4700-bbbbbbbbbbbb"),
            &ctx(1_000)
        )
        .action,
        PeerAction::NoAction
    ...
    let r = m.check(
        "d",
        &t,
        &peer("10.0.0.3", "-qB4700-cccccccccccc"),
        &ctx(2_000),
    );
    assert_eq!(r.action, PeerAction::Ban);
...
fn multi_dialing_counts_distinct_peers_per_subnet_and_torrent() {
    ...
    for now in [0, 1_000, 2_000] {
        assert_eq!(
            m.check(
                "d",
                &t,
                &peer("10.0.0.1", "-qB4700-aaaaaaaaaaaa"),
                &ctx(now)
            )
            .action,
            PeerAction::NoAction
    ...
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("10.0.1.1", "-qB4700-aaaaaaaaaaaa"),
            &ctx(3_000)
        )
        .action,
        PeerAction::NoAction
    ...
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("10.0.1.2", "-qB4700-aaaaaaaaaaaa"),
            &ctx(4_000)
        )
        .action,
        PeerAction::NoAction
    ...
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("10.0.2.1", "-qB4700-aaaaaaaaaaaa"),
            &ctx(later)
        )
        .action,
        PeerAction::NoAction
    ...
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("10.0.2.2", "-qB4700-aaaaaaaaaaaa"),
            &ctx(later + 1_000)
        )
        .action,
        PeerAction::NoAction
    ...
    assert_eq!(
        m.check(
            "d",
            &t,
            &peer("10.0.2.3", "-qB4700-aaaaaaaaaaaa"),
            &ctx(later + 2_000)
        )
        .action,
        PeerAction::Ban
...
fn multi_dialing_uses_ipv6_tolerance_and_prefix() {
    ...
    let r = m.check(
        "d",
        &t,
        &peer("2001:db8:1::6", "-qB4700-aaaaaaaaaaaa"),
        &ctx(6_000),
    );
    assert_eq!(r.action, PeerAction::Ban);
    let tr = Translator::embedded();
...
fn multi_dialing_keep_hunting_bans_following_peers_within_window() {
    ...
    let r = m.check(
        "d",
        &t,
        &peer("10.0.0.9", "-qB4700-aaaaaaaaaaaa"),
        &ctx(100_001),
    );
    assert_eq!(r.action, PeerAction::Ban);
    let tr = Translator::embedded();
    ...
    let expired = m.check(
        "d",
        &t,
        &peer("10.0.0.77", "-qB4700-aaaaaaaaaaaa"),
        &ctx(100_001 + 600_000),
    );
